...
<body>
				...
				<div class="row">
					<div class="col-lg-12">
						<div
								...
								<div class="col-xs-1" style='padding-left: 5px'>
									<span><strong>商品分类</strong></span> <select id='category'
										...
										<c:forEach var='cate' items='${cates}'>
											<c:if test="${cate.c_id==currentcid }">
									...
									<tbody>
										<c:forEach var='p' items='${pageBean.list}'>
											<tr class="odd gradeX">
												...
												<td>${p.p_price }</td>
												<td>${p.p_addtime }</td>
												<td><a href='editProductUI?p_id=${p.p_id }&currentCid=${currentcid}&page=${pageBean.currentPage}' class='btn btn-info'>修改商品</a> <a
													href='deleteProduct?p_id=${p.p_id }&c_id=${currentcid}&page=${pageBean.currentPage}' class='btn btn-danger'>删除商品</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
							...
							<div class="row">
								<div class="col-xs-12 text-right" style="overflow: hidden,margin-right:30px;">
									...
									<nav aria-label="Page navigation" style="float: right">
										<ul class="pagination">
											...
											<c:if test="${pageBean.currentPage!=1 }">
												<li><a
													href="${pageContext.request.contextPath }/showProductUI?cateid=${currentcid}&currentPage=${pageBean.currentPage-1}"
											...
											<c:forEach var="page" items="${pageBean.pageValue }">
												...
												<c:if test="${pageBean.currentPage!=page && page!=-1 }">
													<li><a
														href="${pageContext.request.contextPath }/showProductUI?cateid=${currentcid}&currentPage=${page}">${page }</a>
													</li>
												</c:if>
											</c:forEach>
											<!-- 最后一页的逻辑 -->
	...
	<script>
		$('#category')
				.change(
						function() {
							window.location.href = '${pageContext.request.contextPath}/showProductUI?cateid='
									+ this.value;
						});
		...
		function changePage() {
			...
			location.href = "${pageContext.request.contextPath }/showProductUI?cateid=${currentcid}&currentPage="
					+ $("#inputNumber").val();
		}
	</script>
	<script>
